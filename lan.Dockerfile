...
RUN set -x \
  ...
  && apk update \
  && apk add --no-cache nodejs npm  bash vim  python python-dev gcc libcurl make\
  && rm -rf /var/cache/apk/* \
  && mkdir /yapi && cd /yapi \
  && wget --no-check-certificate http://192.168.1.100/yapi-1.8.0.tar.gz \
  && tar -zxf yapi-1.8.0.tar.gz \
  && mv yapi-1.8.0 /yapi/vendors \
  && rm -rf *.tar.gz \
